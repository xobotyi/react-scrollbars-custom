...
export default class Scrollbar extends React.Component {
  ...
  static propTypes = {
    ...
    trackVerticalClassName: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.array
    ]),
    trackHorizontalClassName: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.array
    ]),
    thumbVerticalClassName: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.array
    ]),
    thumbHorizontalClassName: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.array
    ]),
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      ...
      if (
        !this.props.noScroll &&
        (this.props.onScrollStart || this.props.onScrollStop)
      ) {
   ...
   */
  addListeners = (
    noScrollChanged = false,
    noScrollYChanged = false,
    noScrollXChanged = false
  ) => {
    ...
    if (noScrollChanged || noScrollYChanged) {
      if (noScrollY) {
        this.trackVertical.removeEventListener(
          "mousedown",
          this.handleTrackVerticalMousedownEvent
        );
        this.thumbVertical.removeEventListener(
          "mousedown",
          this.handleThumbVerticalMousedownEvent
        );
      } else {
        this.trackVertical.addEventListener(
          "mousedown",
          this.handleTrackVerticalMousedownEvent
        );
        this.thumbVertical.addEventListener(
          "mousedown",
          this.handleThumbVerticalMousedownEvent
        );
      }
    ...
    if (noScrollChanged || noScrollXChanged) {
      if (noScrollX) {
        this.trackHorizontal.removeEventListener(
          "mousedown",
          this.handleTrackHorizontalMousedownEvent
        );
        this.thumbHorizontal.removeEventListener(
          "mousedown",
          this.handleThumbHorizontalMousedownEvent
        );
      } else {
        this.trackHorizontal.addEventListener(
          "mousedown",
          this.handleTrackHorizontalMousedownEvent
        );
        this.thumbHorizontal.addEventListener(
          "mousedown",
          this.handleThumbHorizontalMousedownEvent
        );
      }
  ...
  handleTrackVerticalMousedownEvent = e => {
    ...
    const offset =
      ...
      this.thumbVertical.clientHeight / 2;

    this.content.scrollTop = this.computeScrollTopForThumbOffset(
      Math.max(offset, 0)
    );
  };
  ...
  handleTrackHorizontalMousedownEvent = e => {
    ...
    const offset =
      ...
      this.thumbHorizontal.clientWidth / 2;

    this.content.scrollLeft = this.computeScrollLeftForThumbOffset(
      Math.max(offset, 0)
    );
  };
  ...
  handleThumbVerticalMousedownEvent = e => {
    ...
    this.dragPrevPageY =
      target.clientHeight - (clientY - target.getBoundingClientRect().top);
  ...
  handleThumbHorizontalMousedownEvent = e => {
    ...
    this.dragPrevPageX =
      target.clientWidth - (clientX - target.getBoundingClientRect().left);
  ...
  computeThumbVerticalHeight = trackHeight => {
    const height = Math.ceil(
      (this.content.clientHeight / this.content.scrollHeight) * trackHeight
    );

    return trackHeight === height
      ? 0
      : Math.max(height, this.props.minimalThumbsSize);
  };
  ...
  computeThumbHorizontalWidth = trackWidth => {
    ...
    return trackWidth === width
      ? 0
      : Math.max(width, this.props.minimalThumbsSize);
  };
  ...
  computeScrollLeftForThumbOffset = offset => {
    ...
    return (
      (offset /
        (trackHorizontalInnerWidth - this.thumbHorizontal.clientWidth)) *
  ...
  update = (forced = false, rtlAutodetect = false) => {
    ...
    !isset(isRtl) &&
      (isRtl = rtlAutodetect
        ? getComputedStyle(this.content).direction === "rtl"
        : this.isRtl || false);
    ...
    this.trackVertical.style.display =
      verticalScrollPossible ||
      this.props.permanentScrollbars ||
      this.props.permanentScrollbarY
        ? null
        : "none";
    ...
    this.trackHorizontal.style.display =
      horizontalScrollPossible ||
      this.props.permanentScrollbars ||
      this.props.permanentScrollbarX
        ? null
        : "none";
    ...
    if (verticalScrollPossible) {
      const trackVerticalInnerHeight = getInnerHeight(this.trackVertical);
      const thumbVerticalHeight = this.computeThumbVerticalHeight(
        trackVerticalInnerHeight
      );
    ...
    (this.scrollValues || false) &&
      this.props.onScroll &&
      this.props.onScroll(currentScrollValues, this);
  ...
  render() {
          ...
          style: {
            ...
            overflowY:
              noScroll || noScrollY
                ...
                : "auto",
            direction:
              (rtl === true && "rtl") || (rtl === false && "ltr") || null
          },
          ref: ref => {
            this.holder = ref;
          },
    ...
    const holderClassNames =
        "ScrollbarsCustom-holder" + (className ? " " + className : ""),
      wrapperClassNames =
      ...
      thumbVerticalClassNames =
        "ScrollbarsCustom-thumb ScrollbarsCustom-thumbVertical" +
    ...
    if (noScroll || (noScrollX && noScrollY)) {
      ...
      !browserScrollbarWidth &&
        (contentStyles.paddingRight = contentStyles.paddingBottom = null);
